...
def handle_key_hook(e: tuple) -> None:
    ...
    if e.event_type == "down":
        ...
        if e.name == RUBBER_KEY:
            window.evaluate_js("global.setItem('rubber')")
        elif e.name == LASER_KEY:
            window.evaluate_js("global.setItem('laser')")
            is_laser = True
        elif e.name == PEN_KEY:
            window.evaluate_js("global.setItem('pen')")
        elif e.name == CIRCLE_KEY:
            window.evaluate_js("global.setItem('circle')")
        elif e.name == TRIANGLE_KEY:
            window.evaluate_js("global.setItem('triangle')")
        elif e.name == SQUARE_KEY:
            window.evaluate_js("global.setItem('square')")
        elif e.name == RECTANGLE_KEY:
            window.evaluate_js("global.setItem('rectangle')")
            ...
            window.evaluate_js("global.clearAll()")
    else:
        if e.name == "shift":
